...
import { deleteToken } from '../../store/slices/user';
...
const Header = ({children, resized=false}: IProps) => {
  const location = useLocation();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const isMainPage = ['/', '/team'].includes(location.pathname);
  const controlsNames = createLongClassName([styles.controls].concat(isMainPage ? [styles.controls_oneElem] : []).concat(resized ? [styles.resized] : []));

  const onMainPage = (e: React.MouseEvent) => {
    navigate('/');
  }
  ...
  return ( 
    <header className={createLongClassName(['white-color', 'purple-bgc', styles.header].concat([resized ? [styles.header_resized] : []]))}>
      <div className='container'>
        <div className={styles.contentWrap}>
          <div className={controlsNames}>
            {
              !isMainPage && 
              <>
                <BorderBtn additionalClassNames={[styles.largeScreenBtn]} onClick={onMainPage}>
                  Назад
                </BorderBtn>
                <button className={createLongClassName(['animated-btn', styles.mobileScreenBtn])} onClick={onMainPage}>
            ...
            <BorderBtn additionalClassNames={[styles.largeScreenBtn]} onClick={onLogout}>
              Выход
            </BorderBtn>
            <button className={createLongClassName(['animated-btn', styles.mobileScreenBtn])} onClick={onLogout}>
              <svg width='40' height='40' viewBox='0 0 40 40' fill='none' xmlns='http://www.w3.org/2000/svg'>
